...
<?
	...
	private function th($caption, $width=false, $right = false) {
		echo '<th';
...
*/
	...
	private function klassifikation() {
		...
		foreach ($klass as $row) {
			...
			echo '<tr><td>Orden</td><td>'.$row['Orden'].'</td><td>'.$this->dash($row['Orden_dk']).'</td></tr>';
			$count--;
			if ($count>0) {
	...
	private function referencer() {
		$this->header();
		echo '<table style="max-width:480px;width:480px;"><thead>';
		$this->th('Navn', 80);
		$this->th('År', 40);
		$this->th('Titel', 200);
		$this->th('Arter', 30, true);
		echo '</thead><tbody>';
		...
		while ($r = mysql_fetch_assoc($result)) {
			...
			*/
			$SQL='select Referencetekst, Reference_aar, '.
				"(select count(*) from allearter where Artsgruppe='".$this->artsgruppe."' and Referencenavn='".$ref."') as arter ".
				' from allearter where Referencenavn="'.$ref.'" and Artsgruppe="'.$this->artsgruppe.'"';
